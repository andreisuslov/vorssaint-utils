...
enum ClipboardHistoryEscape {
    ...
    /// own toggle turns it off, and it is the only thing that does now that
    /// Space marks a row for batch rather than opening the pane (see
    /// `ClipboardHistorySpaceKey`), so a keystroke meant to close the panel
    /// can never silently re-hide it first.
